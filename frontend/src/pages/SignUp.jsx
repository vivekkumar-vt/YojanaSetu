...
const SignUp = () => {
  ...
  const inputCls = 'w-full rounded-2xl border border-white/10 bg-white/5 text-white px-4 py-3.5 text-sm placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all';
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">
              ...
              <Link to="/signin" className="font-bold text-indigo-400 hover:text-indigo-300 transition-colors">Sign in here</Link>
            </p>
          </div>
          ...
          <div className="flex justify-center gap-4 mb-6 flex-wrap">
            {perks.map(p => (
              <span key={p} className="flex items-center gap-1.5 text-xs font-semibold text-indigo-300">
                <CheckCircle2 className="w-3.5 h-3.5 text-indigo-400" /> {p}
              </span>
            ))}
          </div>
          ...
          <div className="bg-white/[0.06] backdrop-blur-2xl rounded-3xl border border-white/10 p-8 shadow-2xl">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                <label htmlFor="aadhaar" className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
                  Aadhaar Number <span className="text-slate-600 normal-case font-medium">(Optional)</span>
                </label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-500 hover:text-slate-300 transition-colors">
                      {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </button>
                  </div>
                </div>
                <div>
                  <label htmlFor="confirmPassword" className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Confirm</label>
            ...
            <div className="mt-5 pt-5 border-t border-white/10 text-center">
              <Link to="/" className="text-sm font-medium text-slate-500 hover:text-white transition-colors">← Back to Home</Link>
            </div>
          </div>

          <p className="mt-6 text-center text-xs text-slate-600 flex items-center justify-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5 text-indigo-500" /> Secure registration · Your data is always protected
          </p>
        </motion.div>
      </div>
    </div>
