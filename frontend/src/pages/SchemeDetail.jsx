    ...
    <div className="mesh-bg min-h-screen pb-24">
        ...
        <div className="max-w-6xl mx-auto relative z-10 pt-10">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-12">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="flex-shrink-0">
               <div className="glass-card p-4 rounded-[2.5rem] flex flex-col gap-3 shadow-2xl border-white/40">
                  <button 
                    onClick={handleSave}
                    className={`flex items-center justify-center gap-3 px-10 py-5 rounded-[2rem] font-black text-lg transition-all duration-300 ${
                      isSavedLocal 
                        ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-400/40' 
                        : 'bg-white text-indigo-600 border-2 border-indigo-50 hover:border-indigo-200'
                    }`}
                  >
                    {isSavedLocal ? <BookmarkCheck className="w-6 h-6" /> : <Bookmark className="w-6 h-6" />}
                  ...
                  <a
                    ...
                    className="flex items-center justify-center gap-3 px-10 py-5 rounded-[2rem] bg-emerald-600 text-white font-black text-lg shadow-xl shadow-emerald-600/30 hover:bg-emerald-500 hover:shadow-emerald-600/50 hover:-translate-y-1 transition-all no-underline"
                  >
                    Apply Now <ExternalLink className="w-6 h-6" />
                  </a>
                  <div className="flex items-center justify-center gap-2 mt-2 pt-2 border-t border-indigo-50">
                     <button className="p-3 rounded-2xl text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all" aria-label="Share this scheme"><Share2 className="w-5 h-5" /></button>
                     <div className="w-px h-6 bg-slate-100" />
                     <button className="p-3 rounded-2xl text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all" aria-label="More information"><Info className="w-5 h-5" /></button>
                  </div>
               </div>
            </motion.div>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8 -mt-8 relative z-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          >
            {/* Description */}
            <motion.div variants={itemVariants} className="bg-white rounded-[3rem] p-10 lg:p-14 shadow-xl border border-slate-50 transition-colors">
              <div className="flex items-center gap-4 mb-8">
                 <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center shadow-inner">
                    <BookOpen className="w-6 h-6 text-indigo-500" />
                 </div>
                 <h2 className="text-3xl font-black text-slate-900" style={{ fontFamily: 'Outfit, sans-serif' }}>About the Scheme</h2>
              </div>
              <p className="text-xl text-slate-600 leading-[1.8] font-medium mb-10">
                {scheme.description}
              </p>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                 <div className="p-8 rounded-[2.5rem] bg-indigo-50/50 border border-indigo-100/50">
                    <Sparkles className="w-8 h-8 text-indigo-500 mb-4" />
                    <h4 className="font-black text-slate-900 mb-2">Scheme Status</h4>
                    <p className="text-sm text-slate-600 font-bold uppercase tracking-widest text-indigo-600">Active & Rolling</p>
                 </div>
                 <div className="p-8 rounded-[2.5rem] bg-pink-50/50 border border-pink-100/50">
                    <Calendar className="w-8 h-8 text-pink-500 mb-4" />
                    <h4 className="font-black text-slate-900 mb-2">Launch Date</h4>
                    <p className="text-sm text-slate-600 font-bold uppercase tracking-widest text-pink-600">Updated Jan 2024</p>
                 </div>
              </div>
            </motion.div>

            {/* Benefits */}
            <motion.div variants={itemVariants} className="bg-white rounded-[3rem] p-10 lg:p-14 shadow-xl border border-slate-50">
               <div className="flex items-center gap-4 mb-10">
                 <div className="w-12 h-12 rounded-2xl bg-emerald-50 flex items-center justify-center shadow-inner">
                    <Award className="w-6 h-6 text-emerald-500" />
                 </div>
                 <h2 className="text-3xl font-black text-slate-900" style={{ fontFamily: 'Outfit, sans-serif' }}>Key Benefits</h2>
              </div>
              <div className="space-y-6">
                {scheme.benefits?.map((benefit, i) => (
                  <motion.div 
                    ...
                    className="flex items-start gap-5 p-6 rounded-3xl bg-slate-50 border border-slate-100/50 hover:bg-emerald-50/30 hover:border-emerald-100 transition-all group"
                  >
                    <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0 mt-1 group-hover:scale-110 transition-transform">
                      <CheckCircle2 className="w-5 h-5 text-emerald-600" />
                    </div>
                    <span className="text-lg text-slate-700 font-bold leading-relaxed">{benefit}</span>
                  </motion.div>
                ))}
              </div>
            </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="space-y-8">
            ...
            <div className="bg-indigo-600 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group">
              ...
              <h3 className="text-2xl font-black mb-8 relative z-10" style={{ fontFamily: 'Outfit, sans-serif' }}>Who Can Apply?</h3>
              <div className="space-y-6 relative z-10">
                 <div className="flex items-start gap-4">
                    <div className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center mt-1"><MapPin className="w-3.5 h-3.5" /></div>
                    <div>
                       <p className="text-xs font-bold text-white/60 uppercase tracking-widest mb-1">State Eligibility</p>
                       <p className="font-black">PAN India / Central</p>
                    </div>
                 </div>
                 <div className="flex items-start gap-4">
                    <div className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center mt-1"><Info className="w-3.5 h-3.5" /></div>
                    <div>
                       <p className="text-xs font-bold text-white/60 uppercase tracking-widest mb-1">Target Group</p>
                       <p className="font-black">{scheme.eligibility}</p>
                    </div>
                 </div>
            ...
            {/* Quick Actions */}
            <div className="glass-card p-10 rounded-[3rem] border-white/60">
               <h3 className="text-xl font-black text-slate-900 mb-6" style={{ fontFamily: 'Outfit, sans-serif' }}>Helpful Resources</h3>
               <div className="space-y-4">
                  <a href="#" className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 hover:bg-indigo-50 transition-colors group">
                    <div className="flex items-center gap-3">
                       <FileText className="w-5 h-5 text-indigo-500" />
                       <span className="text-sm font-bold text-slate-700">Official Circular</span>
                    </div>
                  ...
                  <a href="#" className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 hover:bg-indigo-50 transition-colors group">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-sm font-bold text-slate-700">Participating Banks</span>
                    </div>
                    <ArrowRight className="w-4 h-4 text-slate-400 group-hover:text-indigo-600 transition-colors" />
                  </a>
               </div>
